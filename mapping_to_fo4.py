...
with open(sys.argv[1]) as json_file:
        ...
        if this_port["direction"] == 'input':
            ...
            for pin in pins:
                if not pin in connections:
                    print(
                        "Warning: Module's input pin %d is not connected to any cell's input" % pin)
                    continue
    ...
    while len(list_of_cells_to_propagate) > 0:
        ...
        for cell in list_of_cells_to_propagate:  # For all cells which had an output propagated to them
            ...
            for cell_output_pin in list_cell_outputs(cell):
                ...
                if cell_output_pin_open or is_pin_output(cell_output_pin):
                    print("Info: %s output pin %d is not connected to any cell's input" % (
                        cell["name"], cell_output_pin))

                    # error detection
                    if cell_output_pin in output_pin_max_fo4:
                        print(
                            "Error: We are already done processing output pin %d but you try to duplicate it's entry" % cell_output_pin)
    ...
    print("Pin | max FO4 | max chain | min F04 | min chain | name")
    for i in output_pin_last_cell:
        print("{: >3} | {:7.2f} | {: >9} | {:7.2f} | {: >9} | {}".format(
            i, output_pin_max_fo4[i], output_pin_max_cell[i], output_pin_min_fo4[i],
            output_pin_min_cell[i], output_pin_last_cell[i]["name"]
        ))
    ...
    for i in output_pin_max_fo4:
        if output_pin_max_fo4[i] > max_fo4:
            ...
            max_fo4_index = i
    print("Max FO4: {} from cell {}".format(
        max_fo4, output_pin_last_cell[max_fo4_index]["name"]))
    print(
    ...
    for i in output_pin_min_fo4:
        if output_pin_min_fo4[i] < min_fo4:
            ...
            min_fo4_index = i
    print("Min FO4: {} from cell {}".format(
        min_fo4, output_pin_last_cell[min_fo4_index]["name"]))
    print(
